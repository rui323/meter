...
class NrsiwataController : public cnoid::SimpleController {
    cnoid::Link *t_link, *b_link, *crawlerL, *crawlerR;
    ...
    // float Velocity =0.01;
    float Velocity = 0.0; // 0.0 ,0.1, 0.01, 0.001
   ...
   public:
    virtual bool initialize( cnoid::SimpleControllerIO* r_io ) override {  
        ...
        crawlerL = io->body()->link("L_crawler");
        crawlerR = io->body()->link("R_crawler");
        crawlerL->setActuationMode(cnoid::Link::JOINT_SURFACE_VELOCITY);
        io->enableOutput(crawlerL);
        crawlerR->setActuationMode(cnoid::Link::JOINT_SURFACE_VELOCITY);
    ...
    virtual bool control( ) override {
        ...
        if (isRecording) {
            ...
            crawlerL->dq_target() = Velocity;
            crawlerR->dq_target() = Velocity;
        } else {
            crawlerL->dq_target() = 0.0;
            crawlerR->dq_target() = 0.0;
        }
        return true;
    }
